...
def updater():
    ...
    latestCommit = get(
        'https://raw.githubusercontent.com/VKUsual18/XSSFinder/refs/heads/main/core/config.py').text
    ...
    if changes not in latestCommit:  # just a hack to see if a new version is available
        ...
        logger.good('A new version of XSS Finder is available.')
        changes_str = 'Changes:\n'
        ...
        if choice != 'n':
            logger.run('Updating XSS Finder')
            os.system(
                'git clone --quiet https://github.com/VKUsual18/XSSFinder %s' % (folder))
            os.system('cp -r %s/%s/* %s && rm -r %s/%s/ 2>/dev/null' %
                      (path, folder, path, path, folder))
            logger.good('Update successful!')
    else:
        logger.good('XSS FInder is up to date!')
